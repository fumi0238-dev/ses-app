  ...
  try {
    ...
    const record = await prisma.project.update({
      ...
      data: {
        ...
        client_price_min: body.client_price_min !== undefined ? safeParseInt(body.client_price_min) : undefined,
        client_price_max: body.client_price_max !== undefined ? safeParseInt(body.client_price_max) : undefined,
        purchase_price_min: body.purchase_price_min !== undefined ? safeParseInt(body.purchase_price_min) : undefined,
        purchase_price_max: body.purchase_price_max !== undefined ? safeParseInt(body.purchase_price_max) : undefined,
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    const now = BigInt(Date.now());
    // 案件を削除 + 関連マッチングもカスケード削除
    const relatedMatchings = await prisma.matching.findMany({
      where: { project_id: id, deleted: false },
      select: { id: true },
    });
